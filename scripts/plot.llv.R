...
#  if(!file.exists(folder)) { stop(paste("ERROR: Folder does not exist:",folder)) }
  ...
  ## Set filenames #TODO comment seems out of date?
  plot.single.llvfolds <- FALSE

  ## Return gracefully if the llv output files don't exist
  print('Loading LLV results'); flush.console()
  if(!( file.exists(file.path(folder,"perf_platypus_expanded.tab")) & file.exists(file.path(folder,"perf_llv_expanded.tab")) ) ){ stop('ERROR: missing LLV output files'); flush.console() } 
...
## TODO: this is only used if weighting is turned on in the LLV run. So should only use it if that's true- also it's not actually used anywhere below, so that's a problem! Dig out the old code and fix this
  ## Set up PLATYPUS labelling info
  ## Accuracy, coverage, and thresholds of PLATYPUS labelling
#  ll.acc.avg   <- c() # TODO: Where is this supposed to be used???
#  ll.acc.sd    <- c() # TODO: Where is this supposed to be used???
#  ll.cov.avg   <- c() # TODO: Where is this supposed to be used???
#  ll.cov.sd    <- c() # TODO: Where is this supposed to be used???
#  ll.upper.avg <- c() # TODO: Where is this supposed to be used???
#  ll.upper.sd  <- c() # TODO: Where is this supposed to be used???
#  ll.lower.avg <- c() # TODO: Where is this supposed to be used???
#  ll.lower.sd  <- c() # TODO: Where is this supposed to be used???
#
#  ## Calculate values
